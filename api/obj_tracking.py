...
class YOLOV5_SORT:
    def __init__(self, weights: str, img_size: Tuple[int], classes, tracking: bool = False):
    ...
    def infer(self, source):
        ...
        for path, img_1, img_2, vid_cap, string in minibatch:
            ...
            for i, det in enumerate(pred):
                p, img_3, frame = path, img_2.copy(), getattr(minibatch.count, "frame", 0)
                if len(det):
                    ...
                    if self.tracking:
                        ...
                        for x1, y1, x2, y2, conf, detclass in det.cpu().detach().numpy():
                            dets_to_sort = np.vstack((dets_to_sort, np.array([x1, y1, x2, y2, detclass])))
